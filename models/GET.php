...
if(!$answer1['Privlage']){
    ...
    public function CheckCreator($Project, $login)
    {
        ...
        $query->execute(['ID' => $Project]);
        $answer = $query->fetch(PDO::FETCH_ASSOC);
	if ($answer && $answer['ID_WORK'] == $login){
            return true;
        } else {
            return false;
        }
